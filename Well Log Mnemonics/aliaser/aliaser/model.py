    ...
    """
        ...
        if self.num_directions > 1:
            ...
            hidden = (
                ...
                .contiguous()
                .view(1, batch_size, self.hidden_size * self.num_directions)
            )
...
class DecoderRNN(nn.Module):
    def __init__(
        ...
        *,
        ...
        if (
            tied_embedding is not None
            and self.out_embed_size
            and embed_size != self.out_embed_size
        ):
            print(
    ...
    """
        ...
        if self.enc_attn or self.pointer:
            ...
            enc_energy = self.enc_bilinear(
                hidden.expand(num_enc_steps, batch_size, -1).contiguous(),
                encoder_states,
            )
            if self.enc_attn_cover and coverage_vector is not None:
                enc_energy += self.cover_weight * torch.log(
        ...
        if self.dec_attn:
            if decoder_states is not None and len(decoder_states) > 0:
                dec_energy = self.dec_bilinear(
                    hidden.expand_as(decoder_states).contiguous(), decoder_states
                )
        ...
        if self.pointer:
            ...
            gen_output = F.softmax(
                logits, dim=1
            )  # can't use log_softmax due to adding probabilities
            output[:, : self.vocab_size] = prob_gen * gen_output
            # add pointer probabilities to output
            ptr_output = enc_attn
            output.scatter_add_(
                1, encoder_word_idx.transpose(0, 1), prob_ptr * ptr_output
            )
            if log_prob:
...
class Seq2SeqOutput(object):
    def __init__(
        ...
        loss: Union[torch.Tensor, float] = 0,
        loss_value: float = 0,
        enc_attn_weights: torch.Tensor = None,
        ...
        self.enc_attn_weights = (
            enc_attn_weights  # (out seq len, batch size, src seq len)
        )
...
class Seq2Seq(nn.Module):
    ...
    """
        ...
        if vocab.embeddings is not None:
            ...
            if params.embed_size is not None and self.embed_size != params.embed_size:
                print(
                    "Warning: Model embedding size %d is overriden by pre-trained embedding size %d."
        ...
        self.max_dec_steps = (
            params.max_tgt_len + 1 if max_dec_steps is None else max_dec_steps
        )
        ...
        self.decoder = DecoderRNN(
            ...
            self.embed_size,
            dec_hidden_size,
            enc_attn=params.enc_attn,
            dec_attn=params.dec_attn,
            pointer=params.pointer,
            out_embed_size=params.out_embed_size,
            tied_embedding=self.embedding if params.tie_embed else None,
            in_drop=params.dec_in_dropout,
            rnn_drop=params.dec_rnn_dropout,
            out_drop=params.dec_out_dropout,
            enc_hidden_size=enc_total_size,
        )
    ...
    """
        ...
        log_prob = not (
            sample or self.decoder.pointer
        )  # don't apply log too soon in these cases
        ...
        if saved_out:  # reuse encoder states of a previous run
            ...
            encoder_outputs, encoder_hidden = self.encoder(
                encoder_embedded, encoder_hidden, input_lengths
            )
        ...
        for di in range(target_length):
            ...
            if enc_attn_weights:
                ...
                coverage_vector = None
            decoder_output, decoder_hidden, dec_enc_attn, dec_prob_ptr = self.decoder(
                decoder_embedded,
                decoder_hidden,
                encoder_outputs,
                torch.cat(decoder_states) if decoder_states else None,
                coverage_vector,
                encoder_word_idx=input_tensor,
            ...
            if not sample:
                _, top_idx = decoder_output.data.topk(
                    1
                )  # top_idx shape: (batch size, k=1)
            else:
                prob_distribution = (
                    torch.exp(decoder_output) if log_prob else decoder_output
                )
            ...
            if criterion:
                ...
                if not log_prob:
                    decoder_output = torch.log(
                        decoder_output + eps
                    )  # necessary for NLLLoss
            ...
            if self.enc_attn_cover or (criterion and self.cover_loss > 0):
                if coverage_vector is not None and criterion and self.cover_loss > 0:
                    coverage_loss = (
                        torch.sum(torch.min(coverage_vector, dec_enc_attn))
                        / batch_size
                        * self.cover_loss
                    )
            ...
            if use_teacher_forcing or (
                use_teacher_forcing is None and random.random() < forcing_ratio
            ):
    ...
    """
        ...
        step = 0
        while (
            hypos and step < 2 * max_out_len
        ):  # prevent infinitely generating punctuations
            ...
            if (
                self.dec_attn and step > 0
            ):  # dim 0 is decoding step, dim 1 is beam batch
            ...
            top_v, top_i = decoder_output.data.topk(
                beam_size
            )  # shape of both: (beam size, beam size)
