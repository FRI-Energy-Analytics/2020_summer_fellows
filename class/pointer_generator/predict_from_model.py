...
from utils import rouge, Vocab, OOVDict, Batch, format_tokens, Dataset
...
def eval_bs(test_set: Dataset, vocab: Vocab, model: Seq2Seq, params: Params):
  ...
  if params.test_save_results and params.model_path_prefix:
    result_file = tarfile.open("results.tgz", 'w:gz')
  output = {}
  model.eval()
  for _ in range(1, n_samples + 1):
    batch = next(test_gen)
    predicted, mnem = eval_bs_batch(batch, model, vocab, pack_seq=params.pack_seq,
